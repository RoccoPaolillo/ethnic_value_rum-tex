...
author: 
    ...
    | **` Fig 4 shows ß secondary = ß dominant * w (df_secf.csv) `**
    | **` Fig 4 could substitute fig. 5 and 7. Only 3 levels w shown`**
    | **` Annex A shows all level of w`**
    |  
...
<!--
```{r secfdom1, fig.width = 6,fig.height= 8, out.width="80%",  fig.pos = "H", fig.cap="Baseline condition, ß secondary as function of ß dominant. Comparison increase in ß ethnic liberal or ß value conservative", warning=FALSE}

# Facet by outcome

lib_fsec <- df_secf_pic %>% filter(ID == "lib") %>% filter(lib_eth == 0 | lib_eth == 0.5 | lib_eth == 1) %>% 
  ggplot(aes(x = dominant,y = Segra_bsl, color = outcome, shape = orientation, linetype = as.factor(lib_eth))) + 
  geom_point(size = 1.5) + geom_line() +
  facet_wrap(~ outcome, dir = "v", labeller = as_labeller(c("Ethnic" = "Ethnic Segregation", "Value" = "Value Segregation", "Density" = "Density"))) +
  ylab("")+
  xlab("ß dominant") +
  scale_color_manual(values = c("Ethnic" = "purple","Value" = "dark green","Density"= "gray"))+
  scale_shape_manual(values = c("Conservative" = "square","Liberal" = "circle"), guide = guide_legend(title = "Orientation")) +
  scale_linetype_manual(values = c("0" = "dotted", "0.5" = "dashed","1" = "solid"), guide = guide_legend(title = "weight ß secondary")) +
  ggtitle("ß ethnic liberal") +
  theme_bw() + guides(color = FALSE)

con_fsec <- df_secf_pic %>% filter(ID == "con") %>% filter(con_val == 0 | con_val == 0.5 | con_val == 1) %>% 
  ggplot(aes(x = dominant,y = Segra_bsl, color = outcome, shape = orientation, linetype = as.factor(con_val))) + 
  geom_point(size = 1.5) + geom_line() +
  facet_wrap(~ outcome, dir = "v", labeller = as_labeller(c("Ethnic" = "Ethnic Segregation", "Value" = "Value Segregation", "Density" = "Density"))) +
  ylab("")+
  xlab("ß dominant") +
  ...
  scale_shape_manual(values = c("Conservative" = "square","Liberal" = "circle"), guide = guide_legend(title = "Orientation")) +
  scale_linetype_manual(values = c("0" = "dotted", "0.5" = "dashed","1" = "solid"), guide = guide_legend(title = "weight ß secondary")) +
  ggtitle("ß value conservative") +
  theme_bw() + guides(color = FALSE)

secf_pic <- ggpubr::ggarrange(lib_fsec,con_fsec,common.legend = TRUE, legend  = "bottom")

secf_pic
```
-->

```{r secfdom, fig.width = 8,fig.height= 8, out.width="100%",  fig.pos = "H", fig.cap="Baseline condition, ß secondary as function of ß dominant. Comparison increase in ß ethnic liberal or ß value conservative", warning=FALSE}
...
# Annex A: ß secondary = ß dominant * w


```{r secfdom_all, fig.width = 10,  fig.pos = "H", fig.cap="Baseline condition, ß secondary as function of ß dominant. Comparison increase in ß ethnic liberal or ß value conservative", warning=FALSE}
