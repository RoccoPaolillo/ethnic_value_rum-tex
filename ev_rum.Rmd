...
```{r fig.1,include = TRUE,  warning = FALSE}

dfpic <- df_bsl_pic %>% filter(ID == "basic" ) %>% filter(con_val == 1 & lib_eth == 1 | con_val == 0 & lib_eth == 0)
dfpic$fig1 <- ifelse(dfpic$con_val == 0 & dfpic$lib_eth == 0,"w[sec]==0", "w[sec]==1") %>% factor(levels = c("w[sec]==1","w[sec]==0"))


ggplot(dfpic,aes(x = dominant, y = Segra_bsl, color = outcome_bsl, shape = orientation_bsl)) +
  geom_point(size = 1.5) + geom_line() +
  facet_wrap(~ fig1, dir = "v", labeller = label_parsed) + # labeller(fig1 = fig1.labs))+
  ...
  theme_bw() 
ggsave("images/fig1.jpg",width = 6,height = 5)
```
...
# ggsave("images/totsens.jpg",width = 7,height = 7)
...
ggsave("images/totsens2.jpg",width = 7,height = 5)
...
scale_color_manual(values = brewer.pal(11, "Spectral"))+
 ...
 ggsave("images/libeth.jpg",width = 7, height = 4)
...
guides(shape = guide_legend(title = "Exposure",order = 2), color = guide_legend(title = "w ethnic\nliberal migrant",order = 1))
 ggsave("images/asym_native.jpg",width = 6,height = 5)
...
ggplot(aes(x = dominant, y = Segra_asm,shape = outcome, color = as.factor(eth_lib_min)
 ...
 scale_color_manual(values = c("0" = "orange","0.5" = "red","1" = "brown"))+
  ...
  theme_bw() +
guides(shape = guide_legend(title = "Exposure",order = 2), color = guide_legend(title = "w ethnic\nliberal migrant",order = 1))
ggsave("images/asym_migrant.jpg",width = 6,height = 5)
...
scale_color_manual(values = brewer.pal(11, "Spectral"))+
  ...
  ggsave("images/conval.jpg",width = 7, height = 4)
```
...
guides(shape = guide_legend(title = "Outcome",order = 2), color = guide_legend(title = "w ethnic\nliberal migrant",order = 1))
 ggsave("images/asym_nativedensity.jpg",width = 6,height = 5)
...
ggplot(aes(x = dominant, y = Segra_asm,shape = outcome, color = as.factor(eth_lib_min)
 ...
 scale_color_manual(values = c("0" = "orange","0.5" = "red","1" = "brown"))+
  ...
  theme_bw() +
guides(shape = guide_legend(title = "Outcome",order = 2), color = guide_legend(title = "w ethnic\nliberal migrant",order = 1))
 ggsave("images/asym_migrantdensity.jpg",width = 6,height = 5)
...
ggsave(pic, file = paste0("images/",sbj,"_vlsz.jpg"), width = 6, height = 8)
}
...
picet <- annotate_figure(ggpubr::ggarrange(et,vl, common.legend = T, legend = "bottom",nrow = 2), 
                top = text_grob(sbj, size = 12))
ggsave(picet, file = paste("images/",sbj,"_etsz.jpg"), width = 6, height = 9)
# ggpubr::ggarrange(et,vl, common.legend = T, legend = "bottom")
}
