...
```{r asym_equal_orig,fig.align='center',fig.pos = "H", fig.cap=, warning=FALSE}
# 
# et_sq_or_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = et_sq_or_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","et_sq_or_m",".jpg"))
# 
# et_sq_bl_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = et_sq_bl_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","et_sq_bl_m",".jpg"))
# 
# vl_sq_or_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = vl_sq_or_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","vl_sq_or_m",".jpg"))
# 
# vl_sq_bl_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = vl_sq_bl_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","vl_sq_bl_m",".jpg"))
# 
# ##
# 
# et_cl_or_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = et_cl_or_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","et_cl_or_m",".jpg"))
# 
# et_cl_bl_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = et_cl_bl_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","et_cl_bl_m",".jpg"))
# 
# vl_cl_or_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = vl_cl_or_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","vl_cl_or_m",".jpg"))
# 
# vl_cl_bl_m <- df %>% filter(ID == "valuesize" ) %>% ggplot(aes(x = dominant, y = vl_cl_bl_m, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
#   labeller = label_both)
# ggsave(paste0("images/","vl_cl_bl_m",".jpg"))
# 
# et_sq <- ggpubr::ggarrange(et_sq_or_m,et_sq_bl_m, common.legend = TRUE, legend = "bottom")
# vl_sq <- ggpubr::ggarrange(vl_sq_or_m,vl_sq_bl_m, common.legend = TRUE, legend = "bottom")
# or_sq <- ggpubr::ggarrange(et_sq_or_m,vl_sq_or_m, common.legend = TRUE, legend = "bottom")
# bl_sq <- ggpubr::ggarrange(et_sq_bl_m,vl_sq_bl_m, common.legend = TRUE, legend = "bottom")
# 
# et_cl <- ggpubr::ggarrange(et_cl_or_m,et_cl_bl_m, common.legend = TRUE, legend = "bottom")
# vl_cl <- ggpubr::ggarrange(vl_cl_or_m,vl_cl_bl_m, common.legend = TRUE, legend = "bottom")
# or_cl <- ggpubr::ggarrange(et_cl_or_m,vl_cl_or_m, common.legend = TRUE, legend = "bottom")
# bl_cl <- ggpubr::ggarrange(et_cl_bl_m,vl_cl_bl_m, common.legend = TRUE, legend = "bottom")
# 
# et_sq
# vl_sq
# et_cl
...
seg_vis <- function(db,var_seg,var_seg_y,var_seg_tit){
 
pic <-  db %>% ggplot(aes(x = dominant, y = var_seg, color = as.factor(liberal_maj ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
  labeller = label_both) +
 ylab(var_seg_y) +
  xlab("ß dominant") +
  scale_color_manual("% Liberal Blue / % Liberal Orange", values = c("50" = "coral2", "60" = "olivedrab", "70" = "cyan", "80" = "violet"), 
                     labels = c("50" = "50/50", "60" = "60/40", "70" = "70/30", "80" = "80/20")
                     
                     ) +
  labs(title = "Equal ethnic size" , subtitle = var_seg_tit) +
  # ggtitle(var_seg_tit) +
  # ggsubtitle(rel_siz) +
  theme_bw() +
...
et_sq_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$et_sq_bl_m,"Ethnic Exposure", "Conservative Blue")
vl_sq_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$vl_sq_bl_m,"Value Exposure", "Conservative Blue")
et_cl_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$et_cl_bl_m,"Ethnic Exposure", "Liberal Blue")
vl_cl_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$vl_cl_bl_m,"Value Exposure", "Liberal Blue")

et_sq_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$et_sq_or_m,"Ethnic Exposure", "Conservative Orange")
vl_sq_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$vl_sq_or_m,"Value Exposure", "Conservative Orange")
et_cl_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$et_cl_or_m,"Ethnic Exposure", "Liberal Orange")
vl_cl_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$vl_cl_or_m,"Value Exposure", "Liberal Orange")

cls_et_sq_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_et_sq_bl_m,"Ethnic Cluster", "Conservative Blue")
cls_vl_sq_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_vl_sq_bl_m,"Value Cluster", "Conservative Blue")
cls_et_cl_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_et_cl_bl_m,"Ethnic Cluster", "Liberal Blue")
cls_vl_cl_bl_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_vl_cl_bl_m,"Value Cluster", "Liberal Blue")

cls_et_sq_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_et_sq_or_m,"Ethnic Cluster", "Conservative Orange")
cls_vl_sq_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_vl_sq_or_m,"Value Cluster", "Conservative Orange")
cls_et_cl_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_et_cl_or_m,"Ethnic Cluster", "Liberal Orange")
cls_vl_cl_or_m <- seg_vis(df[df$ID == "valuesize",],df[df$ID == "valuesize",]$cls_vl_cl_or_m,"Value Cluster", "Liberal Orange")


## majority/minority condition


seg_vis_mj <- function(db,var_seg,var_seg_y,var_seg_tit){
 
pic <-  db %>% ggplot(aes(x = dominant, y = var_seg, color = as.factor(majority ))) + geom_point() + geom_line() + facet_grid( eth_lib_min ~ eth_lib_maj, scales = "free_y", 
  labeller = label_both) +
 ylab(var_seg_y) +
  xlab("ß dominant") +
  scale_color_manual("% Ethnic majority / % Ethnic minority", values = c("50" = "coral2", "60" = "olivedrab", "70" = "cyan", "80" = "violet"), 
                     labels = c("50" = "50/50", "60" = "60/40", "70" = "70/30", "80" = "80/20")
                     
                     ) +
  labs(title = "Majority (blue)/Minority (orange)" , subtitle = var_seg_tit) +
  # ggtitle(var_seg_tit) +
  # ggsubtitle(rel_siz) +
  theme_bw() +
...
mj_et_sq_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$et_sq_bl_m,"Ethnic Exposure", "Conservative Majority")
mj_vl_sq_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$vl_sq_bl_m,"Value Exposure", "Conservative Majority")
mj_et_cl_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$et_cl_bl_m,"Ethnic Exposure", "Liberal Majority")
mj_vl_cl_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$vl_cl_bl_m,"Value Exposure", "Liberal Majority")

mj_et_sq_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$et_sq_or_m,"Ethnic Exposure", "Conservative Minority")
mj_vl_sq_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$vl_sq_or_m,"Value Exposure", "Conservative Minority")
mj_et_cl_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$et_cl_or_m,"Ethnic Exposure", "Liberal Minority")
mj_vl_cl_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$vl_cl_or_m,"Value Exposure", "Liberal Minority")

mj_cls_et_sq_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_et_sq_bl_m,"Ethnic Cluster", "Conservative Majority")
mj_cls_vl_sq_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_vl_sq_bl_m,"Value Cluster", "Conservative Majority")
mj_cls_et_cl_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_et_cl_bl_m,"Ethnic Cluster", "Liberal Majority")
mj_cls_vl_cl_bl_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_vl_cl_bl_m,"Value Cluster", "Liberal Majority")

mj_cls_et_sq_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_et_sq_or_m,"Ethnic Cluster", "Conservative Minority")
mj_cls_vl_sq_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_vl_sq_or_m,"Value Cluster", "Conservative Minority")
mj_cls_et_cl_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_et_cl_or_m,"Ethnic Cluster", "Liberal Minority")
mj_cls_vl_cl_or_m <- seg_vis_mj(df[df$ID == "ethnicsize",],df[df$ID == "ethnicsize",]$cls_vl_cl_or_m,"Value Cluster", "Liberal Minority")
...
# Annex A2: Asymmetric conditions: ß secondary as function of ß dominant

Model: e_v_rum_secfdom.nlogo


The aim is to observe what difference occurs if majority or minority group holds ß secondary (ethnic similarity preference) as function of ß dominant (value similarity):
...
```{r et_lib_asm, fig.align='center',fig.pos = "H", out.width="100%",fig.cap="ß ethnic liberals as function of ß value liberals, range w [0,1] ", warning=FALSE}

library(ggpubr)


sec_etlib <- read.csv("secfdom_data/sec_etlib.csv") %>% select(
  ID,density,majority,liberal_maj,liberal_min,dominant_distribution,secondary_distribution,dominant,secondary,con_eth,con_val,lib_val,lib_eth,eth_con_maj,val_con_maj,eth_con_min,val_con_min,val_lib_maj,eth_lib_maj,val_lib_min,eth_lib_min,step,
  cls_et_cl_bl,cls_et_cl_or,cls_et_sq_bl,cls_et_sq_or,cls_vl_cl_bl,cls_vl_cl_or,cls_vl_sq_bl,cls_vl_sq_or) %>% group_by(ID, density,majority,liberal_maj,liberal_min,dominant_distribution,secondary_distribution,dominant,secondary,con_eth,con_val,lib_val,lib_eth,eth_con_maj,val_con_maj,eth_con_min,val_con_min,val_lib_maj,eth_lib_maj,val_lib_min,eth_lib_min,step )  %>% 
  summarise_all(c("m"= mean,"sd"= sd)) 
  
  
sec_etlibpic <- sec_etlib %>%  gather(key = group_asm, value = Segra_asm, cls_et_cl_bl_m,cls_et_cl_or_m,cls_et_sq_bl_m,cls_et_sq_or_m,cls_vl_cl_bl_m,cls_vl_cl_or_m,cls_vl_sq_bl_m,cls_vl_sq_or_m)  %>%
  mutate(`outcome` = fct_relevel(fct_collapse(group_asm,
                                              "Ethnic Segregation" = c("cls_et_cl_bl_m","cls_et_cl_or_m","cls_et_sq_bl_m","cls_et_sq_or_m"),
                                              "Value Segregation" = c("cls_vl_cl_bl_m","cls_vl_cl_or_m","cls_vl_sq_bl_m","cls_vl_sq_or_m")),"Ethnic"),
         `type` = fct_relevel(fct_collapse(group_asm,
                                                  "Conservative Majority" = c("cls_et_sq_bl_m","cls_vl_sq_bl_m"),
                                                  "Conservative Minority" = c("cls_et_sq_or_m","cls_vl_sq_or_m"),
                                                  "Liberal Majority" = c("cls_et_cl_bl_m","cls_vl_cl_bl_m"),
...
for(f in c("Conservative Majority","Conservative Minority","Liberal Majority","Liberal Minority")){
majj <-  sec_etlibpic %>% filter(type == f & ID == "sec_etlbmaj") %>% ggplot(aes(
  x = dominant,
  y = Segra_asm, 
  color = as.factor(eth_lib_maj))) +
  geom_point(size = 1.5) + geom_line()  +
  facet_wrap(~outcome, ncol = 1, scales = "free_y", labeller = as_labeller( c("Ethnic Segregation" = "Ethnic Segregation", "Value Segregation" = "Value Segregation" ,"sec_etlbmaj" = "ß eth liberal majority: \nß value * w"))) +
   scale_color_manual(values = brewer.pal(11, "Spectral"), guide = guide_legend(title = "w")) + 
  xlab("ß dominant") + 
  ylab("Clustering") +
  labs(title = "ß ethnic liberal majority",
       subtitle = "ß value * w" ) +
  theme_bw() +
   theme(plot.title = element_text(size = 12, hjust = 0.5), plot.subtitle = element_text(size = 11, hjust = 0.5),
          plot.background = element_rect(colour = "black", fill=NA, size=0.5), legend.background = element_rect(colour = "black", fill = NA, size = 0.4))

 
  
minn <- sec_etlibpic %>% filter(type == f & ID == "sec_etlbmin") %>% ggplot(aes(
  x = dominant,
  y = Segra_asm, 
  color = as.factor(eth_lib_min))) +
  geom_point(size = 1.5) + geom_line()  +
facet_wrap(~outcome, ncol = 1, scales = "free_y", labeller = as_labeller( c("Ethnic Segregation" = "Ethnic Segregation", "Value Segregation" = "Value Segregation" , "sec_etlbmin" = "ß eth liberal minority: \nß value * w"))) +
   scale_color_manual(values = brewer.pal(11, "Spectral"), guide = guide_legend(title = "w")) +
   xlab("ß dominant") + 
   ylab("Clustering") +
  labs(title = "ß ethnic liberal minority",
       subtitle = "ß value * w" ) +
  theme_bw() +
 theme(plot.title = element_text(size = 12, hjust = 0.5), plot.subtitle = element_text(size = 11, hjust = 0.5),
          plot.background = element_rect(colour = "black", fill=NA, size=0.5), legend.background = element_rect(colour = "black", fill = NA, size = 0.4))
...
picf <- annotate_figure(  ggpubr::ggarrange(majj,minn,common.legend = TRUE, ncol = 2, legend  = "right") , top = text_grob(f, 
                face = "bold", size = 13))


ggsave(paste0(f,".pdf"), picf, path = "ev_rum_files/figure-latex/",width = 10,height = 6,  dpi = 300) 
...
This one \ref{fig: et_lib} in the text would be to be deleted. It combined baseline (w = 0) with either majority or minority holding ß value = ß ethnic (w = 1)
